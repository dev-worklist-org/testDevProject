...
class Dispatcher {
    ...
    static public function loadRoutes() {
        ...
        self::$dispatcher->any('/reports', array('Payments', 'reports'));
        self::$dispatcher->post('/report', array('Payments', 'report'));
    ...
    static public function dispatch() {
        try {
            ...
            $vars =
                ...
                &&  array_key_exists('vars', $route[3])
                    ? $route[3]['vars']
                    : array();
            $default =
                    is_array($route)
                &&  array_key_exists(3 , $route)
                &&  array_key_exists('default', $route[3])
                    ? $route[3]['default']
                    : array();
            ...
            // let's fire an exception if controller not exists
            if (!class_exists($controller)) {
                throw new Exception($controller . " class not found for " . self::$url, 1);
            }
